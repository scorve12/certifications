...
class EventManager {
  ...
  1. 코드에서 메모리 누수가 발생하는 부분을 찾으세요
      - 이벤트 리스너가 DOM 요소와 함께 제거되지 않음
      - 메타데이터가 큰 배열로 인해 메모리 사용량 증가
  2. DOM 요소가 제거될 때 이벤트 리스너도 함께 제거되도록 코드를 수정하세요
      - WeakMap을 사용하여 메모리 누수를 방지할 수 있는 방법을 고려해보세요
      - MutationObserver를 사용하여 DOM 요소 제거를 감지하고 자동으로 리스너를 정리하는 개선된 EventManager를 구현해보세요
